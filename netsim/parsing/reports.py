...
def res_to_table(res):
    ...
    for test_name, cases in res.items():
        ...
        for case, metrics in sorted_cases:
            print(f'| {test_name} | {case} | {metrics["throughput"]:.2f} | {metrics["reported_throughput"]:.2f} |')
...
def res_to_metro(res, commit, integration):
    ...
    for test_name, cases in res.items():
        ...
        suffix = '_'.join(test_name.split('_')[2:] if integration else test_name.split('_')[1:])
        suffix = f'.{suffix}' if suffix else ''
        
        for case, metrics in cases.items():
            name = '_'.join(test_name.split('_')[1:]) if integration else "throughput_gbps"
            tag = f'{case}{suffix}'
            
            r["metrics"].append(create_metric(commit, bucket, name, tag, metrics['throughput'], now))
            if not integration:
                r["metrics"].append(create_metric(commit, bucket, 'reported_throughput_gbps', tag, metrics['reported_throughput'], now))
                if suffix == '':
                    # Report times
                    r["metrics"].extend([
                        create_metric(commit, bucket, 'time', f'{case}{suffix}.total', metrics['elapsed'], now),
                        create_metric(commit, bucket, 'time', f'{case}{suffix}.transfer', metrics['reported_time'], now),
                        create_metric(commit, bucket, 'time', f'{case}{suffix}.setup', metrics['elapsed'] - metrics['reported_time'], now)
                    ])
