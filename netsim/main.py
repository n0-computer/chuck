    ...
    for _ in range(wait_time):
        ...
        for i in range(int(node['count'])):
            ...
            with open(f'logs/{prefix}__{node_name}.txt', 'r') as f:
                for line in f:
                    if node['param_parser'] == 'iroh_ticket' and line.startswith('All-in-one ticket'):
...
def monitor_short_processes(p_short_box, prefix):
    ...
    for node_name, p in p_short_box:
        ...
        if result is None:
            ...
            process_errors.append(f"Process failed: {prefix} with exit code {result} for node {node_name}")
...
def run_case(nodes, runner_id, prefix, args, debug=False, visualize=False):
    ...
    for node in nodes:
        for i in range(int(node['count'])):
            ...
            cmd = handle_connection_strategy(node, node_counts, i, runner_id, node_ips, node_params)

            temp_dir = tempfile.TemporaryDirectory(prefix='netsim', suffix='{}_{}_{}'.format(prefix, node_name, runner_id))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--reports-only", help = "Run only report generation", action='store_true', default=False)
    parser.add_argument("--integration", help = "Run in integration test mode", action='store_true', default=False)
    parser.add_argument("--sniff", help = "Run sniffer to record all traffic", action='store_true', default=False)
    parser.add_argument("--skip", help = "Comma separated list of tests to skip")
    parser.add_argument("--debug", help = "Enable full debug logging", action='store_true', default=False)
    parser.add_argument("--visualize", help = "Enable visualization", action='store_true', default=False)
    parser.add_argument("--max-workers", help = "Max workers for parallel execution", type=int, default=1)
    parser.add_argument("--netsim-log-level", help = "Set log level for netsim", default='error')
    ...
    if os.path.isdir(args.cfg):
        paths = [f for f in glob.glob(os.path.join(args.cfg, '**', '*.json'), recursive=True)]
    else:
