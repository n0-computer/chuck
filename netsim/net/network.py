...
class StarTopo(Topo):
    ...
    def build(self, nodes=[{'name': 'hx', 'count': 1, 'type': 'public'}, {'name': 'h', 'count': 1, 'type': 'public'}], runner_id=0, interconnect='s1'):
        ...
        self.addLink(interconnect, router, intfName2=routerName+'-eth1', params2={ 'ip' : defaultIP } )

        kk = 0
        for node in nodes:
            if node['type'] == 'public':
                for i in range(int(node['count'])):
                    h = self.addHost('%s_%d_r%d' % (node['name'], i, runner_id), cls=EdgeNode, defaultRoute='via 10.1.1.1')
                    if 'link' in node:
                        loss = node['link']['loss']
            ...
            if node['type'] == 'nat':
                ...
                for i in range(int(node['count'])):
                    ...
                    # add NAT to topology
                    nat = self.addNode('n_%s%dr%d' % (node['name'], i, runner_id), cls=NAT, subnet=localSubnet,
                                    inetIntf=inetIntf, localIntf=localIntf)
                    switch = self.addSwitch('natsw%s%dr%d' % (node['name'][:2], i, runner_id))
                    # connect NAT to inet and local switches
                    self.addLink(nat, interconnect, intfName1=inetIntf)
                    self.addLink(nat, switch, intfName1=localIntf, params1=natParams)
                    # add host and connect to local switch
                    host = self.addHost('%s_%d_r%d' % (node['name'], i, runner_id),
        ...
        box = self.addHost('zbox1-r' + str(runner_id), cls=EdgeNode, ip='10.1.1.2', defaultRoute='via 10.1.1.1') # creates a dedicated node to play around
...
class LinuxRouter( Node ):
    def config( self, **params ):
        ...
        self.cmd( 'sleep 1')
        self.cmd( 'smcroutectl -I smcroute-' + self.name +
                  ' add ' + self.name + '-eth1 239.0.0.1 ' + self.name + '-eth2 ' + self.name + '-eth3' )
