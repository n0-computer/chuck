...
class TCIntf(Intf):
    ...
    def delayCmds(parent, delay=None, jitter=None, loss=None, max_queue_size=None):
        ...
        if loss and (loss < 0 or loss > 100):
            ...
            if netemargs:
                cmds = [
                    "%s qdisc add dev %s " + parent + " handle 10: netem " + netemargs
                ]
                parent = " parent 10:1 "
        return cmds, parent
    ...
    def config(
        ...
        **params,
        ...
        stuff = (
            (["%.2fMbit" % bw] if bw is not None else [])
...
class Link(object):
    ...
    def __init__(
        ...
        **params: additional parameters for both interfaces"""
        ...
        if fast:
            ...
            self.makeIntfPair(
                intfName1, intfName2, addr1, addr2, node1, node2, deleteIntfs=False
            )
        else:
    ...
    def makeIntfPair(
        ...
        assert cls
        return makeIntfPair(
            intfname1, intfname2, addr1, addr2, node1, node2, deleteIntfs=deleteIntfs
        )
