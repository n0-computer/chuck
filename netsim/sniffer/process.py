...
def load_pcap(path, keylog=None, topo_path=None):
    ...
    for packet in cap_json:
        ...
        if "ip" in packet:
            ...
            p = {"src": packet.ip.src, "dst": packet.ip.dst, "type": "", "ipv6": False}
    ...
    for ip in ip_set:
        ...
        if ip in topo_node_ip_map:
            ...
            if tn["name"].startswith("i_"):
                n["type"] = "iroh"
                n["id"] = tn["name"].replace("i_", "")
...
class NetsimViz:
    ...
    def run(self, args):
        packets, js_packet_data, node_list = load_pcap(
            args["path"], args["keylog"], args["topo"]
        )
        ...
        self.d = draw.Drawing(
            ...
            animation_config=draw.types.SyncedAnimationConfig(
                self.duration, show_playback_progress=True, show_playback_controls=True
            ),
        )
    ...
    def play(self, packets, nodes):
        ...
        for pp in packets:
            pkts.append(
                self.send_pkt(
                    nodes[self.node_ip_map[pp["src"]]],
                    nodes[self.node_ip_map[pp["dst"]]],
                    pp,
                    self.batch_time,
                    self.batch_interval,
                    i,
    ...
    def draw_background(self):
        bg = draw.Rectangle(
            -self.size_x / 2, -self.size_y / 2, self.size_x, self.size_y, fill="#eee"
        )
        self.d.append(bg)
    ...
    def draw_legend(self, x, y):
        ...
        for k in self.pkt_color_map:
            circle = draw.Circle(
                ...
                y + i * 40,
                self.pkt_size,
                fill=self.pkt_color_map[k],
                stroke=None,
                stroke_width=0,
            )
            label = draw.Text(
                k, 12, x + 8, y + i * 40 + 4, fill="#666", font_family="Helvetica"
            )
            res.append(circle)
            res.append(label)
            i += 1
        ...
        label = draw.Text(
            "IPv6", 12, x + 8, y + i * 40 + 4, fill="#666", font_family="Helvetica"
        )
        res.append(circle)
        res.append(label)
        i += 1
        ...
        label = draw.Text(
            "IPv4", 12, x + 8, y + i * 40 + 4, fill="#666", font_family="Helvetica"
        )
        res.append(circle)
        res.append(label)
        self.attach(res)
    ...
    def draw_node(self, x, y, node):
        res = []
        box = draw.Rectangle(
            x - self.node_size / 2 - self.offset_x,
            y - self.node_size / 2,
            self.node_size,
            self.node_size,
            fill=self.node_color_map[node["type"]],
            rx="4",
            ry="4",
        )
        box_name = draw.Text(
    ...
    def draw_pkt(self, x, y, pkt, t, id):
        ...
        circle = draw.Circle(
            ...
            y,
            0,
            fill=self.pkt_color_map[pkt["type"]],
            stroke=stroke,
            stroke_width=2,
            class_="pkt",
            data_label=pkt["type"],
    ...
    def animate_node_to(self, node, x, y, t):
        node[0].add_key_frame(
            t, x=x - self.node_size / 2 - self.offset_x, y=y - self.node_size / 2
        )
...
def run_viz(args):
    ...
    try:
        sys.stdout = PseudoNonTTY(
            sys.stdout
        )  # disable color output on packet data for JS
